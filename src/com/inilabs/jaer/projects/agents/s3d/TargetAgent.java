...
public class TargetAgent extends AbstractAgent implements  DrawableInSpace3D,  DrawableInPolar,  AgentRunnable,  FBGTarget {
    ...
    public TargetAgent(String key, Space3D.Vec3 p1, Space3D.Vec3 p2, double speedMps){
        super(key, Agent3DTypes.ObjectType.TARGET);
...
public void drawInSpace3D(Graphics2D g2, Space3D world, WorldTransform tx){
    ...
    g2.setColor(AgentColors.colorForKey(getKey()));
    int r = 4;
    g2.fillOval(sx - r, sy - r, 2*r, 2*r);
    g2.setColor(Color.black);
    g2.drawOval(sx - r, sy - r, 2*r, 2*r);

    // draw a short heading stub along current segment direction (if you keep it)
//    Space3D.Vec3 d = p2.sub(p1);
//    double L = d.norm();
//    if (L > 0) {
//        double nx = d.x / L, nz = d.z / L;
//        int sx2 = tx.toScreenX(p.x + 12 * nx);
//        int sy2 = tx.toScreenY(p.z + 12 * nz);
