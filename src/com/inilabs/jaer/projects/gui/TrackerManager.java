...
//		}
    ...
    /**
     * Invokes the polar GUI for Gimbal target tracking. 
 ...
 //           if (bestCluster.getAvgEventRateHzPerPx() >= eventThreshold && bestCluster.getRadius() < sizeThreshold) {
    ...
    private void updateAgentDrawable() {
        assignAgentDrawable();
         if ((agentDrawable != null ) && ( polarSpaceGUI != null )) {
             ...
             agentDrawable.setAzimuth(y);
             agentDrawable.setElevation(p);
    ...
    private void deassignAgentDrawable() {
        if ((agentDrawable != null ) && ( polarSpaceGUI != null )) {
            polarSpaceGUI.getPolarSpaceDisplay().removeDrawable(agentDrawable.getKey()) ;
