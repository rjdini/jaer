 ...
 * @author rjd
 */
public class TrackerManagerExerciser {

    private static final ch.qos.logback.classic.Logger log = (ch.qos.logback.classic.Logger) LoggerFactory.getLogger(TrackerManagerExerciser.class);
...
// Class-level variables to track the azimuth and direction
    ...
    private float offsetAzimuth = -5.0f;
    private float offsetElevation = -10.0f;        
    ...
    private TestCluster testCluster = new TestCluster(0,0, Color.GREEN);

    public TrackerManagerExerciser() {
...
// Generate random azimuth and elevation within a range
    ...
    public LinkedList<TestCluster> getTestClustersHorizontal() {
        ...
        for (int i = 0; i < numberClustersAdded; i++) {
            // Generate random elevation within a range

      //      float azimuth = (float) (Math.random() * 10 - 3); // Range: [-10, 10]
       //     float elevation = (float) (Math.random() * 10 - 3); // Range: [-10, 10]
float azimuth = 0f;
float elevation = 0f;
       
            // Create random testCluster around current azimuth
            testCluster = new TestCluster(offsetAzimuth+currentAzimuth + azimuth, offsetElevation+currentElevation + elevation, Color.GREEN);
