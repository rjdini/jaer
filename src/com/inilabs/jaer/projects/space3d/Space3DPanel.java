 ...
 * with the overlay zoom/pan. The map tile size on screen is dynamically scaled
 * so that meters-per-pixel matches the overlay's pixelsPerMeter.
 *
 * Requirements in Space3D: - getOriginLatDeg(), getOriginLonDeg(),
 * getHalfExtentM()
 *
 * Axes: +X = East (right), +Z = North (up). Units are meters.
 */
public class Space3DPanel extends JPanel {
    ...
    public double getOffsetZM() {
        return offsetZM;
    }

    public void setShowLabels(boolean v) {
        this.showLabels = v;
        repaint();
    }
    ...
    public void setMapZoom(int zoom) {
        this.mapZoom = Math.max(1, Math.min(19, zoom));
        repaint();
    }
    ...
    private double originLat() {
        return space.getOriginLatDeg();
    }

    private double originLon() {
        return space.getOriginLonDeg();
    }
    ...
    /**
     * Draw map tiles covering the viewport, scaled to match overlay
     * pixelsPerMeter
     */
    private void drawMap(Graphics2D g2) {
        ...
        for (int dy = -tilesUp; dy <= tilesDown; dy++) {
            for (int dx = -tilesLeft; dx <= tilesRight; dx++) {
                ...
                if (tx < 0 || ty < 0 || tx >= maxXY || ty >= maxXY) {
                    continue;
                }
    ...
    private static final class TileCache {
        ...
        String key(int z, int x, int y) {
            return z + "/" + x + "/" + y;
        }

        BufferedImage get(int z, int x, int y) {
            return map.get(key(z, x, y));
        }

        void put(int z, int x, int y, BufferedImage img) {
            if (img != null) {
                map.put(key(z, x, y), img);
            }
        }
    }
